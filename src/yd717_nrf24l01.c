...
#define PAYLOADSIZE 9       // receive data pipes set to this size, but unused
...
static void yd717_init()
{
    ...
    else dbgprintf("nRF24L01 detected\n");
...
// TELEMETRY_SetUpdated(TELEM_DSM_FLOG_FRAMELOSS);
	mpx_telemetry[2].value = frameloss; // Appears to be a LQI % value.
}
#endif
...
//    set_rx_tx_addr(lfsr);
    set_rx_tx_addr(Model.fixed_id); // rick.
}
